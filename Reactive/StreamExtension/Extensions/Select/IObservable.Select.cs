...
namespace Soe.Reactive
{
    ...
    static partial class IObservableExtension
    {
        ...
        /// <returns>A representation of a data projection unit</returns>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static Mutator<TIn, TOut, IObservable<TKey, TIn>, SelectProjectionMutator<TIn, TOut>> Select<TKey, TIn, TOut>(this IObservable<TKey, TIn> observable, Func<TIn, TOut> selector)
        {
            return new Mutator<TIn, TOut, IObservable<TKey, TIn>, SelectProjectionMutator<TIn, TOut>>(observable, new SelectProjectionMutator<TIn, TOut>(selector));
        }
    }
}